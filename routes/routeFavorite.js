...
router.post('/addFav', urlencodedParser, async(req, res) => {
    ...
    let username = await favorite.Favorite.findOne({username: req.body.username});
    if (!username) {
        ...
        let userfav = await favorite.Favorite.findOne({username: req.body.username, favorite : req.body.favorite});
        if(!userfav){
            ...
            res.status(400).send('already in fav');
        }
    }
